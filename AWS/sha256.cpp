...
/* These lines have been discarded from the original source code. endian.h is
 * not necessarilly included on the devices that will use this SDK. Little
 * Endian is assumed instead. */
...
void SHA256::getHash(char* result) {
    ...
    // create hash string
    /* Modified from original source code. Creating char* instead of string. */
    char* hashBuffer = new char[HashValues * 8 + 1]();
    size_t offset = 0;
    for (int i = 0; i < HashValues; i++) {
    ...
    hashBuffer[offset] = 0;
    strcpy(result, hashBuffer);
    //return hashBuffer;
}

/* This function added to original source. It is a modified version of
 * getHash() from above. */
/// return latest hash raw (not as hex)
char* SHA256::getHashDec() {
    // save old hash if buffer is partially filled
    uint32_t oldHash[HashValues];
    for (int i = 0; i < HashValues; i++)
        oldHash[i] = m_hash[i];
    ...
    // create hash string
    /* Modified from original source code. Creating char* instead of string. */
    char* hashBuffer = new char[HashValues * 4 + 1]();
    size_t offset = 0;
    for (int i = 0; i < HashValues; i++) {
        hashBuffer[offset++] = (m_hash[i] >> 24) & 0xff;
        hashBuffer[offset++] = (m_hash[i] >> 16) & 0xff;
        hashBuffer[offset++] = (m_hash[i] >> 8) & 0xff;
        hashBuffer[offset++] = m_hash[i] & 0xff;
        // restore old hash
        m_hash[i] = oldHash[i];
    }
    // zero-terminated string
...
/* Modified from original source code. Using char* instead of string. */
void SHA256::operator()(char* result, const void* data, size_t numBytes) {
    reset();
    add(data, numBytes);
    getHash(result);
}
void SHA256::getHashVal(char* result, char* text) {
    reset();
    add(text, strlen(text));
    getHash(&result[0]);
}

void SHA256::getHashValB(char* result, char* text, size_t numBytes) {
    reset();
    add(text, numBytes);
    getHash(result);
}
...
/* These lines have been discarded from the original source code. std::string object
 * is not necessarilly included on the devices that will use this SDK.*/
