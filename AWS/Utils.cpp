...
void hmacSha256(char* digest, const char* key, int keyLen, const char* message,
        int messageLen) {
...
void hexEncode(char* encoded, char* digest, int len) {
	int i;
    encoded[0]='\0';
    for(i=0;i<len;i++) {
	    sprintf(encoded, "%s%02x", encoded,digest[i]);
	}
}
